...
public partial class ClassSessionService
{
    ...
    public async Task<ClassSessionDetailResponse> SubmitReportAsync(int classSessionId, string tutorId, SubmitReportRequest request)
    {
        ...
        try
        {
            ...
            // Nhánh bình thường: buổi đã check-in (cả hai có mặt) → InProgress.
            // Nhánh mới: chỉ gia sư có mặt (đã ghi Istutorpresent=true ở TryAutoCheckInAsync khi
            // học viên không vào phòng) và đã qua giờ kết thúc dự kiến — coi như học viên vắng
            // mặt, cho gia sư tự báo cáo để đi tiếp vào đúng pipeline xác nhận/tự-động-thanh-toán
            // 12h sẵn có (PHHS vẫn có cửa sổ đó để phản đối nếu thấy không hợp lý).
            var isSoloTutorNoShow = classSession.Status == Scheduled
                && classSession.Istutorpresent == true
                && classSession.Isstudentpresent != true
                && now > classSession.Scheduledend;

            // Yêu cầu buổi đã được check-in (in_progress), HOẶC là nhánh học viên vắng mặt ở trên.
            if (classSession.Status != InProgress && !isSoloTutorNoShow)
