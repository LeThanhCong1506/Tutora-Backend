    ...
    public partial class TutorService
    {
        ...
        public async Task<CccdUploadResponse> UploadCccdImagesAsync(string userId, UploadCccdRequest request)
        {
            ...
            if (ocrResult != null)
            {
                ...
                // 5. Kiểm tra số CCCD trùng với tài khoản khác (so sánh ciphertext)
                if (!string.IsNullOrEmpty(ocrResult.Id))
                {
                    var encryptedOcrId = _encryption.Encrypt(ocrResult.Id);
                    if (encryptedOcrId != user.Identitynumber)
                    {
                        var isUnique = await _unitOfWork.UserRepository.IsIdentityNumberUniqueAsync(encryptedOcrId);
                        if (!isUnique)
                            throw new InvalidOperationException(
            ...
            if (ocrResult != null)
            {
                // 7. Mã hóa AES-256 số CCCD trước khi lưu xuống DB
