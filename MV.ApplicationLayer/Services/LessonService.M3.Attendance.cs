...
public partial class LessonService
{
    ...
    public async Task<LessonDetailResponse> CheckInAsync(int lessonId, string tutorId, CheckInRequest request)
    {
        ...
        {
            // Video call: RoomId = lessonId (không cần OAuth, không cần external API)
            lesson.Meetinglink = lessonId.ToString();
            _logger.LogInformation("Assigned video call RoomId {RoomId} for lesson {LessonId}", lessonId, lessonId);
            await _context.SaveChangesAsync();
        }
            ...
            try
            {
                await _notificationService.CreateNotificationAsync(new NotificationRequest
                {
                    ...
                    Message = "Gia sư đã bắt đầu buổi học." +
                                  (hasMeetLink ? " Kiểm tra tin nhắn để lấy Room ID tham gia buổi học." : ""),
